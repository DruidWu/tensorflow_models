...
import sys
sys.path.append('/home/tli15/xiang_wen/soft-nms/lib')
sys.path.append('/home/tli15/xiang_wen/soft-nms/lib/nms')
sys.path.append('/home/tli15/xiang_wen/soft-nms/lib/fast_rcnn')
import nms_wrapper
import numpy as np

np.set_printoptions(threshold=np.nan)

def soft_nms(dets, Nt=0.3, sigma=0.5, threshold=0.001, method=1):
    #print('+++++',dets.shape)
    #print(Nt)
    keep=nms_wrapper.soft_nms_v2(dets, sigma, Nt, threshold, method)
    keep=np.array(keep,np.int32)
    #print(keep)
    return keep
...
def multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
      ...
      ###test
      #boxlist=boxlist_filtered.get()
      #scorelist=boxlist_filtered.get_field(fields.BoxListFields.scores)
      #print(boxlist)
      #print(scorelist)
      #scorelist_ex=tf.expand_dims(scorelist,-1)
      #dets=tf.concat([boxlist,scorelist_ex],axis=-1)
      #print(dets)
      #selected_indices = tf.py_func(soft_nms, [dets,iou_thresh], tf.int32)
      #selected_indices.set_shape(scorelist.get_shape())
      ...
      #selected_indices=tf.contrib.framework.sort(selected_indices)
      #selected_indices = tf.Print(selected_indices, [selected_indices],'selected_indices: ')
